<?php
                    ...
                    if (isset($send_sms) && $send_sms != "none") {
                        ...
                        if ($phone_number != 0) {
                            if ($send_sms == "first_parent") {
                                ...
                                $message_parent_1 = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, $send_sms, $transaction_id);
                                ...
                                $message_student = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, $send_sms, $transaction_id);
                                $phone_student = validateKenyanNumber(explode(",",$phone_number)[0]);
                            }elseif ($send_sms == "both_parent") {
                                $message_category = "parent_account_confirmation_message";
                                $message_parent_1 = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, "first_parent", $transaction_id);
                                $message_parent_2 = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, "second_parent", $transaction_id);
                                ...
                                $message_parent_2 = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, "second_parent", $transaction_id);
                                $message_category = "confirmation_message";
                                $message_student = get_message_template($message_category, $conn2, $studadmin, $amount, $newbalance, "student_contact", $transaction_id);
    ...
    function get_message_template($message_type, $conn2, $adm_no, $amount_paid = 0, $balance = 0, $which_parent = "first_parent", $transaction_id = null)
    {
        ...
        $student_data = getStudentDataSMS($conn2, $adm_no);
        $term = getTerm($conn2);
        if ($which_parent == "first_parent" || $which_parent == "student_contact") {
            for ($index = 0; $index < count($student_data); $index++) {
                if ($student_data[$index]['adm_no'] == $adm_no) {
                    ...
                    $final_message = str_replace("{stud_class}", className($student_data[$index]['stud_class']), $final_message);
                    $dob = date_create($student_data[$index]['D_O_B']);
                    $today = date_create(date("Y-m-d"));
                    $date_diff = date_diff($dob, $today);
                    $date_diff = $date_diff->format("%y Yr(s)");
                    $balance = number_format(getBalance($student_data[$index]['adm_no'], $term, $conn2), 2);
                    ...
                    $time = date("h:i:sa");
                    $final_message = str_replace("{time}", $time, $final_message);
                    ...
                    $final_message = str_replace("{amount_paid}", number_format($amount_paid, 2), $final_message);
                    ...
                    $link = $transaction_id != null ? get_short_url($transaction_id) : "Null";
                    $final_message = str_replace("{receipt_url}", $link, $final_message);
                }
            }
        } else {
            for ($index = 0; $index < count($student_data); $index++) {
    ...
    function getStudentDataSMS($conn2, $adm_no)
    {
        $select = "SELECT * FROM `student_data` WHERE adm_no = '".$adm_no."'";
        $stmt = $conn2->prepare($select);
        $stmt->execute();
    ...
    function get_short_url($transaction_id) {
        
        // encode the url code
        $transaction_id = base64_encode($transaction_id . "");

        // transaction link
        $link = "https://lizola.ladybirdsmis.com/reports/reports.php?receipt=" . $transaction_id . "&p_code=" . base64_encode($_SESSION['databasename']);
        ...
        CURLOPT_POSTFIELDS => json_encode([
            'domain' => 'rcpt.ladybirdsmis.com',
            'allowDuplicates' => false,
            'originalURL' => $link,
            'title' => 'Receipt for transaction '.$transaction_id.' : DB '.$_SESSION['databasename'],
            'tags' => [
            ...
            'utmTerm' => 'sample_term',
            'utmContent' => 'sample_content'
        ]),
        CURLOPT_HTTPHEADER => [
            "Authorization: sk_ZKFr4BFKQ76OwZHs",
        ...
        if (isJson($response)) {
            ...
            return isset($json_response['secureShortURL']) ? $json_response['secureShortURL'] : $link;
        }
        }
        return $link;
    }

    function validateKenyanNumber($number) {
        // Remove spaces, hyphens, and extra characters
        $number = preg_replace('/\D/', '', $number);

        // Check length: must be 10 or 12
        $len = strlen($number);
        if ($len !== 10 && $len !== 12) {
            return null;
        }

        // Case 1: Local format e.g. 07XXXXXXXX or 01XXXXXXXX
        if ($len === 10 && preg_match('/^(07|01)\d{8}$/', $number)) {
            return $number;
        }

        // Case 2: International format e.g. 2547XXXXXXXX or 2541XXXXXXXX
        if ($len === 12 && preg_match('/^(2547|2541)\d{8}$/', $number)) {
            return $number;
        }
