...
use PhpOffice\PhpSpreadsheet\IOFactory;
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET'){
        ...
        if(isset($_GET['admit'])){
            ...
            if(isset($_GET['bynametype'])){
                ...
                if($classenroled != "others"){
                    ...
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $where_clause = "`stud_class` != '-1' AND `stud_class` != '-2' AND `stud_class` != '-3'";
                ...
                createStudentn4($conn2,$result,$searh);
            }elseif (isset($_GET['intake_month'])) {
                $intake_year = $_GET['intake_year'];
                $intake_month = $_GET['intake_month'];
                $start_date = date("Y-m-d", strtotime($intake_year."-".$intake_month."-01"));
                $end_date = date("Y-m-t", strtotime($intake_year."-".$intake_month."-01"));
                $select = "SELECT * from `student_data` WHERE `intake_month` = '$intake_month' AND `intake_year` = '$intake_year'";
                $stmt = $conn2->prepare($select);
            ...
            if ($result) {
                if ($row = $result->fetch_assoc()) {
                    ...
                    if($select_class_id != "daros"){
                        ...
                        $string_to_display.="<option  id='".$value_prefix."-3' value='-3'>Student Enquiries</option>";
                    }
        ...
        if(isset($_POST['admit'])){
            ...
            $send_second_parent = $_POST['send_second_parent'];
            ...
            if($stmt->execute()){
                ...
                if($result){
                    if($row=$result->fetch_assoc()){
                        ...
                        $parentContacts = (strlen(trim($row['parentContacts'])) > 0 && $row['parentContacts'] != null) ? trim($row['parentContacts']) : null;
                        $student_contact = (strlen(trim($row['student_contact'])) > 0 && $row['student_contact'] != null) ? trim($row['student_contact']) : null;
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                $data_to_display = "<select class='form-control' name='".$getmystudents."' id='".$getmystudents."'><option value='' hidden>Select an option</option><option value='-3'>Student Enquiries</option>";
            ...
            if (move_uploaded_file($file["tmp_name"], $fileLocation)) {
                ...
                $found = false;
                $columns = ['student_name','adm_no', 'gender', 'course', 'level', 'module_term', 'study_mode', 'student_contact', 'student_id', 'intake_month', 'intake_year', 'dob', 'doa', 'parent_1_name', 'parent_1_contacts','parent_2_name', 'parent_2_contacts'];
                ...
                for ($row = $first_row; $row <= $highestRow; $row++) {
                    ...
                    $intake_month = $cell(10, $row);
                    $intake_year = $cell(11, $row);
                    $dob = date("Y-m-d", strtotime(explode("|", $cell(12, $row))[0]));
                    $doa = date("Y-m-d", strtotime(explode("|", $cell(13, $row))[0]));
                    $parent_name_1 = $cell(14, $row);
                    $contact_1 = $cell(15, $row);
                    $parent_name_2 = $cell(16, $row);
                    $contact_2 = $cell(17, $row);
                    //--------------------------------------------------------------------------------
                    ...
                    $course_detail = new stdClass();
                    $course_detail->course_level = $level_id;
                    $course_detail->course_name = $course_chosen;
                    $course_detail->course_status = 1;
                    $course_detail->id = 1;
                    $course_detail->module_terms = [];
                    ...
                    // stringify the JSON data
                    $course_details_string = ($level_id != "-1" && $level_id != "-2" && $level_id != "-3") ? json_encode([$course_detail]) : "[]";
                    //--------------------------------------------------------------------------------


                    $new_array = [
                        $student_name, $reg_no, $gender, $course_chosen, $level_id, $course_module_terms,
                        $intake_month, $intake_year, $dob, $doa, 
                        $parent_name_1, $contact_1, $parent_name_2, $contact_2
                    ];
                    $data[] = $new_array;

                    // check admission number or id number is already used
                    $select = "SELECT * FROM student_data WHERE adm_no = '".$reg_no."' OR BCNo = '".$student_id."'";
                    $statement = $conn2->prepare($select);
                    $statement->execute();
                    ...
                    if(count($students) > 0){
                        ...
                        array_push($ommited_students,$student_data);
                        continue;
                    }

                    // prevent "undefined offset"
                    $student_name[1] = $student_name[1] ?? "";
                    $student_name[2] = $student_name[2] ?? "";
                ...
                // ommitted students
                $error_list = "";
                if(count($ommited_students) > 0){
                    $error_list .= "<hr>Students with the following admission number and id number have duplicate data<br><ul>";
                    foreach($ommited_students as $student){
                        $error_list .= "<li class='text-danger'>Admission No: <b>".$student['reg_no']."</b> - <b>National Id:".$student['student_id']."</b></li>";
                    }
                    $error_list.="</ul>";
                }
                ...
                $message = ['success' => true, "message" => "File uploaded successfully!", "file_location" => $fileLocation, "error_list" => $error_list];
                echo json_encode($message);
            } else {
...
function isJson_report($string) {
    ...
    function process_sms($student_data, $message,$adm_no = "0",$conn2,$which_parent){
        ...
        include_once("../finance/financial.php");
        $term = getTermV3($conn2);
        if ($which_parent == "primary" || $which_parent == "student") {
            for ($index=0; $index < count($student_data); $index++) {
                if ($student_data[$index]['adm_no'] == $adm_no) {
                    ...
                    $final_message = str_replace("{stud_class}",myClassName($student_data[$index]['stud_class']),$final_message);
                    ...
                    $final_message = $which_parent == "primary" ? 
                                    str_replace("{title_1}",(((strtolower($student_data[$index]['parent_relation']) == "guardian") ? "":"check") == "check") ? (strtolower($student_data[$index]['parent_relation']) == "father" ? "Mr" : "Mrs"):"",$final_message) : 
                                    str_replace("{title_1}",$student_data[$index]['gender'] == "Male" ? "Mr" : "Mrs",$final_message);
                    $final_message = $which_parent == "secondary" ? 
                                    str_replace("{title_2}",(((strtolower($student_data[$index]['parent_relation']) == "guardian") ? "":"check") == "check") ? (strtolower($student_data[$index]['parent_relation']) == "father" ? "Sir" : "Madam"):"",$final_message) : 
                                    str_replace("{title_2}",$student_data[$index]['gender'] == "Male" ? "Sir" : "Madam",$final_message);
                    $final_message = str_replace("{school_name}",$_SESSION['schname'],$final_message);
                    $final_message = str_replace("{stud_adm}",$student_data[$index]['adm_no'],$final_message);
                    $final_message = str_replace("{school_contact}",$_SESSION['school_contact'],$final_message);
                    $final_message = str_replace("{school_email}",$_SESSION['school_mail'],$final_message);
                    $today = date("D dS M, Y");
    ...
    function trigger_email_agent(){
        $ch = curl_init("https://lawrenzo.ladybirdsmis.com/ajax/sms/email_agent.php?database=".$_SESSION['dbname']."&school_code=".$_SESSION['schoolcode']);
    ...
    function trigger_sms_agent(){
        $ch = curl_init("https://lawrenzo.ladybirdsmis.com/ajax/sms/sms_agent.php?database=".$_SESSION['dbname']."&school_code=".$_SESSION['schoolcode']);
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => false,
            CURLOPT_TIMEOUT_MS     => 200,   // return immediately
            CURLOPT_NOSIGNAL       => 1,
