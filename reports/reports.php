...
class PDF extends FPDF
{
    ...
    // Colored table
    function display_table($header, $data, $width)
    {
        ...
        foreach ($data as $row) {
            foreach ($row as $col) {
                $this->Cell($w[array_search($col, $row)], 6, $col, 1, 0, 'L', $fill);
            }
            $this->Ln();
            $fill = !$fill;
        }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['schname'])) {
    ...
    if (isset($_POST['select_report_class']) && isset($_POST['pdf']) && isset($_POST['select_entity'])) {
        ...
        if ($what_to_print == "examinees_list") {
            ...
            $result = $stmt->get_result();
            if ($result) {
                if ($exam_row = $result->fetch_assoc()) {
                    $exams_names = $exam_row['exams_name'];

                    // GET THE CATS DONE IN THIS EXAM
                    $select = "SELECT * FROM exams_cat WHERE exams_cat.exam_id = ?";
                    $stmt = $conn2->prepare($select);
                    $stmt->bind_param("s", $exam_ids_printing);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $cats = [];
                    if($result){
                    ...
                    if (count($student_data) > 0) {
                        ...
                        $pdf = new PDF('P', 'mm', 'A4');
                        $pdf->setHeaderPos(200);
                        $tittle = "" . ucwords(strtolower($exams_names. " - " . className_exam($classes_for_exams) . " in $course_name Examinees List."));
                        $pdf->set_document_title($tittle);
                        $pdf->setSchoolLogo("../../" . schoolLogo($conn));
                        $pdf->set_school_name($_SESSION['schname']);
                        $pdf->set_school_po($_SESSION['po_boxs']);
                ...
                if ($exam_row = $result->fetch_assoc()) {
                    ...
                    if($result){
                        ...
                        while($row = $result->fetch_assoc()){
                            ...
                            foreach($cats as $cat){
                                $cat_score = get_student_cat_score($conn2, $cat['cat_id'], null, $row['adm_no']);
                                array_push($each_stud, $cat_score['cat_score']." Mks");
                                if($cat['include_in_exam'] == "1"){
                                    $total_cat_marks += $cat_score['cat_score'];
                                    $total_cat_max_marks += $cat['max_marks'];
                                }
                            }
                            if($what_to_print == "exams_marks"){
                                $exam_score = get_student_exam_score($conn2, $exam_ids_printing, $_POST['course_units_for_exams'], null, $row['adm_no']);
                                array_push($each_stud, $exam_score['exam_score'], ($total_cat_marks+$exam_score['exam_score'])."%");
                            }else{
                    ...
                    $stmt = $conn2->prepare($select);
                    $stmt->bind_param("s", $_POST['course_units_for_exams']);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    if($result){
                        if($row = $result->fetch_assoc()){
                            $unit_name = $row['display_name']." {".$row['timetable_id']."}";
                        }
                    }
                    ...
                    if (count($student_data) > 0) {
                        ...
                        array_push($header, "Exam (".(100-$total_cat_max_marks)." Mks)", "Total");
                        array_push($width, 40, 40);
                        $pdf->display_table($header, $student_data, $width);
                        $pdf->Output();
                    } else {
                        echo "<p style='color:red;'><b>Note: </b><br>No student were available in that class during this exams occurence!</p>";
            ...
            include("../connections/conn2.php");
            include("../connections/conn1.php");
            $exam_id = $_POST['exam_ids_printing'];
            $course_level = $_POST['classes_for_exams'];
            ...
            if (count($student_data) > 0) {
                ...
                $pdf = new PDF('P', 'mm', 'A4');
                $pdf->setHeaderPos(200);
                $tittle = "" . ucwords(strtolower($exams_names. " - " . className_exam($course_level) . " in $course_name Examinees List" ));
                $pdf->set_document_title($tittle);
                $pdf->setSchoolLogo("../../" . schoolLogo($conn));
                $pdf->set_school_name($_SESSION['schname']);
                $pdf->set_school_po($_SESSION['po_boxs']);
...
function print_A5_receipt($receipt_id, $database)
{
    $receipt_id = base64_decode($_GET['receipt']);
    $database = base64_decode($_GET['p_code']);
    ...
    $select = "SELECT * FROM `finance` WHERE `transaction_id` = '" . $receipt_id . "'";
    $stmt = $conn2->prepare($select);
    $stmt->execute();
    $result = $stmt->get_result();
    $finance_data = [];
    if ($result) {
        if ($row = $result->fetch_assoc()) {
            array_push($finance_data, $row);
        }
    }

    // check if the finance data is present
    if (count($finance_data) > 0) {
        // finance data
        $finance_data = $finance_data[0];

        // get student data
        $select = "SELECT * FROM `student_data` WHERE `adm_no` = '" . $finance_data['stud_admin'] . "'";
        $stmt = $conn2->prepare($select);
        $stmt->execute();
        $student_data = [];
        $result = $stmt->get_result();
        ...
        // display error is the student data is invalid

        if (count($student_data) == 0) {
            echo "<p style='color:red;'>Invalid URL, contact the school and try again!</p>";
            return 0;
        }
        ...
        // get the school data
        $school_data = [];
        $select = "SELECT * FROM `school_information` WHERE `database_name` = '" . $database . "'";
        $stmt = $conn->prepare($select);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result) {
            if ($row = $result->fetch_assoc()) {
                array_push($school_data, $row);
            }
        }
        ...
        $pdf = new PDF2('P', 'mm', 'A4');
        $pdf->setHeaderPos(200);
        $tittle = ucwords(strtolower($student_data['first_name'] . " " . $student_data['second_name'])) . " Fees Receipt.";
        $pdf->set_document_title($tittle);
        $pdf->setSchoolLogo("../../" . $school_data['school_profile_image']);
        $pdf->set_school_name($school_data['school_name']);
        $pdf->set_school_po($school_data['po_box']);
        $pdf->set_school_box_code($school_data['box_code']);
        $pdf->set_school_contact($school_data['school_contact']);
        $pdf->AddPage();
        $pdf->SetMargins(5, 5);

        // get the school information
        // $school_info = getSchoolInfo($conn);
        $school_name = ucwords(strtoupper($school_data['school_name']));
        $school_motto = ucwords(strtolower($school_data['school_motto']));
        $school_admin_name = $school_data['school_admin_name'];
        $school_mail = $school_data['school_mail'];
        $county = $school_data['county'];
        $physicall_address = $school_data['physicall_address'];
        $country = $school_data['country'];
        $school_profile_image = "../" . $school_data['school_profile_image'];
        $po_box = $school_data['po_box'];
        $box_code = $school_data['box_code'];
        $school_contact = $school_data['school_contact'];
        $website_name = $school_data['website_name'];

        $students_names = ucwords(strtolower($student_data['first_name'] . " " . $student_data['second_name']));
        $pdf->Image($school_profile_image, 5, 10, 20, 20);
        $pdf->SetFont('Helvetica', 'B', 14);
        $pdf->SetFillColor(100, 100, 100);
        $pdf->SetTitle("Receipt for " . $students_names . " Reg No. " . $student_data['adm_no'] . ".");
        $pdf->Cell(15, 10, "", 0, 0, "L", false);
        ...
        $pdf->Cell(25, 6, "Receipt No. : ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(60, 6, $finance_data['transaction_id'], 1, 0, "L");
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(20, 6, "Date : ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(30, 6, date("D dS-M-Y", strtotime($finance_data['date_of_transaction'])), 1, 0, "L", false);

        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(25, 6, "Time : ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(40, 6, date("H:i:sA", strtotime($finance_data['time_of_transaction'])), "RTB", 1, "L", false);
        // $pdf->Cell(53, 6, "", "RBT", 1, "L");

        // student data
        $class_name = classNameReport($student_data['stud_class']);

        // row 2
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(30, 6, "Student Name ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(55, 6, $students_names, 1, 0, "L");
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(20, 6, "Adm No", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(30, 6, $student_data['adm_no'], 1, 0, "L");
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(25, 6, "Class ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(40, 6, $class_name, 1, 1, "L");

        // THIRD ROW
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(20, 6, "Amount", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(25, 6, $finance_data['amount'], 1, 0, "L");
        $new_numbers = new NUmbers();
        $new_number = returnNumbers("Kes " . $finance_data['amount']) * 1;
        $my_number = $new_number < 0 ? $new_numbers->convert_number($new_number * -1) : $new_numbers->convert_number($new_number);
        $prefix = $new_number < 0 ? "Negative " : "";
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(155, 6, "** " . $prefix . " " . $my_number . " Kenya Shillings Only **", "BR", 1, "L");

        // voteheads paid for
        $pdf->SetFillColor(240, 240, 240);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(155, 7, "VOTEHEADS PAID FOR", "TBLR", 0, "C", true);
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(15, 7, "Prev Bal:", "TBLR", 0, "C", false);

        // display voteheads
        $payments_for = $finance_data['payment_for'];
        $display_name = $payments_for;

        // get the amount paid to affect the balance
        $amount_paid = 0;
        if (isJson_report($display_name)) {
            $payments_for = json_decode($payments_for, true);
            for ($index = 0; $index < count($payments_for); $index++) {
                // get provisional payment
                if ($payments_for[$index]['roles'] == "provisional") {
                    continue;
                }

                // add to the amount paid if not provisional
                $amount_paid += $payments_for[$index]['amount_paid'];
            }
        } else {
            if ($payments_for[$index]['roles'] != "provisional") {
                $amount_paid += ($finance_data['amount'] * 1);
            }
        }
        $pdf->Cell(30, 7, "Kes " . number_format(($amount_paid + $finance_data['balance']), 2), "TBLR", 1, "R", false);
        $pdf->SetFont('Helvetica', '', 9);
        $payments_for = $display_name;


        // display voteheads
        $display_name = $finance_data['payment_for'];
        if (isJson_report($display_name)) {
            $payments_for = json_decode($display_name, true);

            for ($index = 0; $index < count($payments_for); $index++) {
                // another row
                $pdf->Cell(20, 7, ($index + 1) . ". ", 1, 0, "L", false);
                $pdf->Cell(110, 7, $payments_for[$index]['name'] != null ? ucwords(strtolower($payments_for[$index]['name'])) : ucwords(strtolower($payments_for[$index]['real_name'])), 1, 0, "L", false);
                $pdf->Cell(25, 7, $payments_for[$index]['roles'], 1, 0, "L", false);
                $pdf->Cell(45, 7, "Kes " . number_format($payments_for[$index]['amount_paid'], 2), "BR", 1, "R", false);
            }
        } else {
            $select = "SELECT * FROM `fees_structure` WHERE `expenses` = '" . $finance_data['payment_for'] . "'";
            $stmt = $conn2->prepare($select);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result) {
                if ($row = $result->fetch_assoc()) {
                    if (strlen($row['display_name']) > 0 && $row['display_name'] != null) {
                        $display_name = ucwords(strtolower($row['display_name']));
                    }
                }
            }
            $pdf->Cell(155, 7, $display_name, 1, 0, "L", false);
            $pdf->Cell(45, 7, "Kes " . number_format($finance_data['amount'], 2), "BR", 1, "R", false);
        }

        // another row
        // $pdf->Cell(155,7,$display_name,1,0,"L",false);
        // $pdf->Cell(45,7,"Kes ". number_format($finance_data['amount']),"BR",1,"L",false);
        ...
        $pdf->Cell(30, 7, "Transaction Code:", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(60, 7, "" . $finance_data['transaction_code'] . "", 1, 0, "L", false);
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(15, 7, "Paid:", 1, 0, "L", false);
        $pdf->Cell(30, 7, "" . "Kes " . number_format($finance_data['amount'], 2) . "", 1, 1, "R", false);
        $pdf->SetFont('Helvetica', '', 9);

        // ANOTHER ROW
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(25, 7, "Payment Mode: ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        // $payment_service = $finance_data['mode_of_pay'] == "mpesa" ? ($finance_data['mpesa_mode_of_payment'] == "mpesa_till_no" ? "(M-Pesa Till)" : ($finance_data['mpesa_mode_of_payment'] == "mpesa_paybill" ? "(M-Pesa Paybill)" : ($finance_data['mpesa_mode_of_payment'] == "mpesa_send_money" ? "(Send Money)" : ($finance_data['mpesa_mode_of_payment'] == "mpesa_pochi" ? "(Pochi La Biashara)" : ""))))  : "";
        $pdf->Cell(40, 7, $finance_data['mode_of_pay'], 1, 0, "L", false);
        $pdf->SetFont('Helvetica', 'B', 9);
        $pdf->Cell(30, 7, "Acc No. : ", 1, 0, "L", true);
        $pdf->SetFont('Helvetica', '', 9);
        $pdf->Cell(30, 7, $student_data['adm_no'], "B", 0, "L", false);
        $pdf->Cell(30, 7, "", "BR", 0, "L", false);

        if (true) {
            $pdf->SetFont('Helvetica', 'B', 9);
            $pdf->Cell(15, 7, "Balance:", 1, 0, "L", false);
            $pdf->Cell(30, 7, "Kes " . number_format($finance_data['balance'], 2) . "", 1, 1, "R", false);
            $pdf->SetFont('Helvetica', '', 9);
        } else {
                ...
                if (isJson_reports($json_data)) {
                    ...
                    for ($index = 0; $index < count($json_data); $index++) {
                        if ($json_data[$index]->show == "true") {
                            ...
                            $pdf->MultiCellWrap(190, 4.5, $json_data[$index]->description, "", 1, "L", false);
                            $counter++;
                        }
                    }
                }
        ...
        $remaining_lines = 30 - $counter;
        $pdf->Ln($remaining_lines);
        $pdf->Cell(200, 0, "", 1, 1);
        $pdf->Ln(5);

        $pdf->Output();
    } else {
        echo "<p style='text-danger'>Invalid URL</p>";
    }
}

function student_data($userid, $conn)
{
    $userid = trim($userid);
...
function isProvisionalFullname($purpose, $conn2, $adm_no = null)
{
    if($adm_no != null){
        $student_data = student_data($adm_no, $conn2);
        if(count($student_data) > 0){
            $votehead_status = $student_data[0]['votehead_status'] ?? null;
            $student_voteheads = isJson_report($votehead_status) ? json_decode($votehead_status) : [];
            foreach($student_voteheads as $votehead){
                if($votehead->class_name == $student_data[0]['stud_class']){
                    if(count($votehead->voteheads) > 0){
                        return "false";
                    }
                }
            }
    ...
    // select statement
    $select = "SELECT `expenses` FROM `fees_structure` WHERE `expenses` = ? AND `roles` = 'provisional'";
    $stmt = $conn2->prepare($select);
...
function get_course_units($conn2, $course_id, $module_number){
    // echo  $course_id." - ".$module_number;
    $select = "SELECT * FROM course_unit_assignment WHERE `course_id` = ? AND `module_number` = ?";
    $stmt = $conn2->prepare($select);
    $stmt->bind_param("ss", $course_id, $module_number);
    $stmt->execute();
    $result = $stmt->get_result();
    $course_units = [];
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            array_push($course_units, $row);
        }
    }
    return $course_units;
}
// get student cat score
function get_student_cat_score($conn2, $cat_id, $examinee_id = null, $student_id = null){
    $cat_score = 0;
    $cat_grade = "N/A";
    $result_id = 0;
    if($examinee_id != null){
        ...
        $select = "SELECT * FROM `exam_cat_scores` WHERE cat_id = ? AND student_adm = ?";
        $stmt = $conn2->prepare($select);
        $stmt->bind_param("ss", $cat_id, $student_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $cat_score = 0;
        $cat_grade = "N/A";
...
function get_student_exam_score($conn2, $exam_id, $unit_id, $examinee_id = null, $student_id = null){
    $exam_score = 0;
    $exam_grade = "N/A";
    $result_id = 0;
    if($examinee_id != null){
        ...
        $select = "SELECT * FROM exam_record_tbl WHERE exam_id = ? AND student_id = ? AND unit_id = ?";
        $stmt = $conn2->prepare($select);
        $stmt->bind_param("sss", $exam_id, $student_id, $unit_id);
        $stmt->execute();
        $result = $stmt->get_result();
        if($result){
            if ($row = $result->fetch_assoc()) {
