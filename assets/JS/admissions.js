...
cObj("message_templates_setup_btn").onclick = function () {
    hideWindow();
    unselectbtns();
    addselected("sms_broadcast");
    cObj("sms_templates_window").classList.remove("hide");
    removesidebar();
...
var sms_school_name = "Ladybird Academy!";
function get_school_information() {
    var datapass = "?getSchoolInformation=true";
    sendData2("GET", "login/login.php", datapass, cObj("school_information_holder") , cObj("class_loader_tags"));
    setTimeout(() => {
        var timeout = 0;
        var ids = setInterval(() => {
            timeout++;
            ...
            if (cObj("class_loader_tags").classList.contains("hide")) {
                var schoolInformation = cObj("school_information_holder").innerText;
                if (hasJsonStructure(schoolInformation)) {
                    var school_details = JSON.parse(schoolInformation);
                    sms_school_name = school_details[1];
                }
                stopInterval(ids);
            }
        }, 100);
...
function select_courses() {
    ...
    if(this.value != "-1" && this.value != "-2" && this.value != "-3"){
        ...
        cObj("student_course_enrolled").classList.remove("hide");
        cObj("course_module_terms_win").classList.remove("hide");
        cObj("student_study_mode").classList.remove("hide");
    }else{
        cObj("student_course_enrolled").classList.add("hide");
        cObj("course_module_terms_win").classList.add("hide");
        cObj("student_study_mode").classList.add("hide");
    }
}
...
cObj("yes_delete_pd").onclick = function () {
    ...
    if (hasJsonStructure(json_data)) {
        var data_index = valObj("get_pd_index") - 1;
...
window.onload = function () {
    ...
    // hide the graphs not to display
    if (cObj("findtrans").classList.contains("d-none")) {
        var finance_graphs = document.getElementsByClassName("finance_graphs");
        for (let index = 0; index < finance_graphs.length; index++) {
            const element = finance_graphs[index];
...
function display_course_list_table(course_history, json_data) {
    ...
    if (cObj("reset_course_term_module") != undefined) {
        cObj("reset_course_term_module").addEventListener("click", function () {
            var datapass = "?reset_course_fees=true&student_adm="+valObj("adminnos");
            sendData2("GET", "administration/admissions.php", datapass, cObj("error_handler_course_progress"), cObj("loadings"), function () {
                cObj("view"+valObj("adminnos")).click();
            });
        });
    }
...
cObj("updatestudinfor").onclick = function () {
    ...
    if (err == 0) {
        if (cObj("course_chosen_edit") != undefined) {
            if (checkBlank("course_chosen_edit") == 1 && (valObj("classed") != "-1" && valObj("classed") != "-2" && valObj("classed") != "-3")) {
...
cObj("submitbtn").onclick = function () {
    ...
    if (errors == 0 && presentBCNO == false) {
        if (typeof (cObj("errolment")) != undefined && cObj("errolment") != null) {
            ...
            var course_chosen = (errolment != "-1" && errolment != "-2" && errolment != "-3") ? valObj("course_chosen") : "";
            var course_module_terms = (errolment != "-1" && errolment != "-2" && errolment != "-3") ? valObj("course_module_terms") : "";
            var module_terms = (errolment != "-1" && errolment != "-2" && errolment != "-3") ? valObj("study_mode") : "";
            ...
            datapass += "&course_chosen="+encodeURIComponent(course_chosen)+"&adm_option="+encodeURIComponent(valObj("automated_amd"));
...
function checkAdmission() {
    ...
    if (cObj("course_module_terms") != undefined) {
        err += (errolment != "-1" && errolment != "-2" && errolment != "-3") ? checkBlank("course_module_terms") : 0;
    }else{
        err += (errolment != "-1" && errolment != "-2" && errolment != "-3") ? 1 : 0;
    }

    if (cObj("course_chosen") != undefined) {
        err += (errolment != "-1" && errolment != "-2" && errolment != "-3") ? checkBlank("course_chosen") : 0;
    }else{
        err += (errolment != "-1" && errolment != "-2" && errolment != "-3") ? 1 : 0;
    }
...
cObj("add_course_btn").onclick = function () {
    ...
    if (err == 0) {
        ...
        var datapass = "?add_course=true&course_name="+encodeURIComponent(valObj("course_input_text"))+"&course_level="+encodeURIComponent(valObj("course_list_setup"))+"&department_name="+encodeURIComponent(dept_name);
        datapass += "&no_of_terms="+encodeURIComponent(valObj("no_of_terms"))+"&fulltime_fees="+encodeURIComponent(valObj("fulltime_fees"))+"&evening_fees="+encodeURIComponent(valObj("evening_fees"))+"&weekend_fees="+encodeURIComponent(valObj("weekend_fees"))+"&term_duration="+encodeURIComponent(valObj("term_duration"))+"&duration_intervals="+encodeURIComponent(valObj("duration_intervals"));
...
cObj("Edit_course_btn").onclick = function () {
    ...
    if (err == 0) {
        ...
        var datapass = "?edit_course=true&course_name="+encodeURIComponent(valObj("course_edit_input_text"))+"&course_levels="+JSON.stringify([])+"&department_name="+encodeURIComponent(dept_name)+"&course_id="+encodeURIComponent(valObj("course_id_holder"));
        datapass += "&course_level="+encodeURIComponent(valObj("course_list_setup_edit"))+"&no_of_terms="+encodeURIComponent(valObj("edit_no_of_terms"))+"&termly_fees="+encodeURIComponent(valObj("edit_termly_fees"))+"&term_duration="+encodeURIComponent(valObj("edit_term_duration"));
        datapass += "&duration_intervals="+encodeURIComponent(valObj("edit_duration_intervals"))+"&fulltime_fees="+encodeURIComponent(valObj("edit_fulltime_fees"))+"&evening_fees="+encodeURIComponent(valObj("edit_evening_fees"))+"&weekend_fees="+encodeURIComponent(valObj("edit_weekend_fees"));
