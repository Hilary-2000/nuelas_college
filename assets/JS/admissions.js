...
function display_course_list_table(course_history, json_data) {
    ...
    if(course_history != null && course_history.module_terms.length > 0){
        ...
        for (let index = 1; index < course_history.module_terms.length; index++) {
            ...
            let delete_button = (index == (course_history.module_terms.length-1)) ? "<span class='btn btn-sm btn-outline-danger' id='delete_course_term_module'><i class='fa fa-trash'></i> Del</span><span class='btn btn-sm btn-outline-success mt-2' id='reset_course_term_module'><i class='fa fa-cog'></i> Reset</span>" : "";
