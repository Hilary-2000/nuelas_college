...
- **All modals live in `homepage.php`**, never nested inside a module's own page file (e.g. `financepages/*.php`, `administration/*.php`). Each module page is wrapped in `<div class="contents animate hide" id="...">`, which stays `display:none` until that module's sidebar button is clicked. A modal nested inside that wrapper stays invisible even after its own `hide` class is removed, since a hidden ancestor hides everything inside it regardless. Modals need to be reachable from other pages too (e.g. a modal opened from a Manage Student action), so they must sit outside every module wrapper, directly in `homepage.php`, alongside the other `.confirmpaymentwindow` modals there.
